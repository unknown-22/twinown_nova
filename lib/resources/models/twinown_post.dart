...
class TwinownPost {
  ...
  String get prettyContent => content.replaceAll(RegExp('<(".*?"|\'.*?\'|[^\'"])*?>'), '');

  String createdAtLocalString() {
    var nowDateTime = DateTime.now().toLocal();
    if (createdAtLocal.year == nowDateTime.year && createdAtLocal.month == nowDateTime.month && createdAtLocal.day == nowDateTime.day) {
